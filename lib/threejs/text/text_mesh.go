...
// NewTextMesh is ...
func NewTextMesh(text string, f *FontTextureAtlas, factoryMethod CharacterMeshFactoryMethod, options []MeshOption) *Mesh {
...
func NewTextSprite(text string, f *FontTextureAtlas, options ...MeshOption) *Mesh {

	var cmfm CharacterMeshFactoryMethod = func(c threejs.Color, ftm *FontTextureMetrics) (CharacterMesh, error) {
		// Material
		material := material.NewSpriteMaterial(map[string]interface{}{
		...
		sprite := sprite.NewSprite(material)
...
func (c *Mesh) Bottom() float64 {
	return c.bottom
}

// createTextMesh is ...
func (c *Mesh) buildTextMesh(text string, meshFactoryMethod CharacterMeshFactoryMethod) error {
